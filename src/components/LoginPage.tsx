...
export default function LoginPage({
    ...
    const handleCredentialSubmit = async () => {
        ...
        if (!user || !password) {
            setAuthError('กรุณากรอกชื่อผู้ใช้หรืออีเมล และรหัสผ่าน');
            return;
        }
    ...
    const handleLinePhoneBinding = async () => {
        ...
        if (!phone) {
            setLineBindingError('กรุณากรอกหมายเลขโทรศัพท์');
            return;
        }
        ...
        try {
            ...
            const message = error instanceof Error ? error.message : 'ผูกเบอร์โทรศัพท์ไม่สำเร็จ';
            ...
            <div className="w-full max-w-[460px]">
                <div className="rounded-2xl border border-[#c9d1dd] bg-white shadow-[0_18px_45px_rgba(15,23,42,0.12)] overflow-hidden">
                    <header className="px-7 py-6 bg-[#0f2740] border-b border-[#0a1d31]">
                        ...
                        <h1 className="mt-3 text-[24px] leading-tight font-bold text-white">เข้าสู่ระบบ Powertec</h1>
                        <p className="mt-1 text-[12px] text-[#c6d5e8]">
                            เข้าสู่ระบบอย่างเป็นทางการสำหรับระบบจัดการงาน
                        </p>
                    </header>
                    ...
                    <main className="px-7 py-6 space-y-3">
                        {allowLineAuth && (
                            <div className="rounded-xl border border-[#d6e2ef] bg-[#f6f9fd] p-3 space-y-2">
                                <p className="text-[12px] font-semibold text-[#1d2936]">เข้าสู่ระบบด้วย LINE</p>

                                {requiresLinePhoneBinding ? (
                                    <>
                                        <p className="text-[12px] text-[#4f5b68]">
                                            บัญชี LINE <span className="font-semibold">{pendingLineProfile?.displayName || '-'}</span> ยังไม่ได้ผูกกับผู้ใช้งานในระบบ
                                            กรุณากรอกหมายเลขโทรศัพท์เพื่อผูกบัญชี
                                        </p>
                                        <input
                                            type="text"
                                            value={linePhone}
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            >
                                                ...
                                                {lineBindingSubmitting ? 'กำลังผูกบัญชี...' : 'ผูกเบอร์โทรศัพท์'}
                                            </button>
                                            <button
                                                type="button"
                                                onClick={loginLine}
                            ...
                            <>
                                <div className="flex items-center justify-between">
                                    <p className="text-[13px] font-semibold text-[#1d2936]">
                                        {authMode === 'login' ? 'เข้าสู่ระบบด้วยชื่อผู้ใช้ / รหัสผ่าน' : 'สร้างบัญชีด้วยชื่อผู้ใช้ / รหัสผ่าน'}
                                    </p>
                                    <button
                                        type="button"
                                        onClick={() => {
                                    ...
                                    >
                                        {authMode === 'login' ? 'สร้างบัญชี' : 'กลับไปเข้าสู่ระบบ'}
                                    </button>
                                </div>
                        ...
                        <div className="rounded-lg bg-[#f5f8fc] border border-[#dde5ef] px-3 py-2">
                            {!lineOnly && (
                                <p className="text-[11px] text-[#4f5b68]">
                                    หมายเหตุ: หากกรอกเฉพาะชื่อผู้ใช้ ระบบจะบันทึกเป็น <code className="font-mono">username@workos.local</code>
                                </p>
                            )}
                            {allowLineAuth && (
                                <p className="mt-1 text-[11px] text-[#6b7785]">
                                    {lineOnly
                                        ? 'หน้านี้รองรับการเข้าสู่ระบบด้วย LINE เท่านั้น หากเป็นการเข้าใช้ครั้งแรก กรุณาผูกด้วยหมายเลขโทรศัพท์ในระบบ'
                                        : 'หน้านี้รองรับการเข้าสู่ระบบด้วย LINE และการผูกหมายเลขโทรศัพท์'}
                                </p>
                            )}
                        </div>
                    </main>
