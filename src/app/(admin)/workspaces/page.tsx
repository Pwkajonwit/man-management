                            ...
                            <tr key={task.id} className={`border-b border-[#e6e9ef] last:border-0 hover:bg-[#f5f6f8] group transition-colors ${activeOwnerDropdown === task.id || activeCrewDropdown === task.id || activeStatusDropdown === task.id || activePriorityDropdown === task.id ? 'relative z-50' : 'relative z-0'} ${overdue ? 'bg-[#fff5f5]' : ''}`}>
                              ...
                              <td className={`px-2 py-0 border-r border-[#d0d4e4] h-[44px] relative ${activeOwnerDropdown === task.id ? 'z-50' : ''}`}>
                                ...
                                {activeOwnerDropdown === task.id && ownerDropdownAnchor && createPortal(
                                  <div
                                    className="fixed z-[220] w-[320px] overflow-hidden rounded-[18px] border border-[#d7deea] bg-white shadow-[0_16px_36px_rgba(15,23,42,0.16)]"
                                    style={{
                                  ...
                                  >
                                    <div className="border-b border-[#e7edf5] bg-[linear-gradient(180deg,#fcfdff_0%,#f6f9fc_100%)] px-3.5 pb-2.5 pt-3">
                                      <div className="flex items-start justify-between gap-3">
                                        <div className="min-w-0">
                                          <div className="text-[13px] font-semibold text-[#2a3647]">กำหนดผู้รับผิดชอบ</div>
                                        </div>
                                        <button
                                          type="button"
                                      ...
                                      <div className="grid grid-cols-5 gap-x-2.5 gap-y-3">
                                        {teamMembers.filter((member) => member.memberType !== 'crew').map((member) => {
                                          ...
                                          return (
                                            ...
                                            >
                                              <div
                                                className={`relative h-11 w-11 rounded-full border-2 bg-white shadow-sm transition-all ${
                                                  isSelected
                                                    ? 'border-[#0073ea] ring-4 ring-[#dbeafe]'
                                                    : 'border-[#d8e0eb] group-hover:border-[#8fbef5] group-hover:shadow-md'
                                                }`}
                                              >
                                                {member.avatar ? (
                                                  <div
                                                  ...
                                                  <div className="flex h-full w-full items-center justify-center rounded-full bg-[radial-gradient(circle_at_top,#f8fafc_0%,#e5eaf2_100%)] text-[12px] font-semibold text-[#334155]">
                                                    {member.name.substring(0, 2).toUpperCase()}
                                                  </div>
                                                )}
                                                {isSelected && (
                                                  <div className="absolute -right-1 -bottom-1 flex h-4.5 w-4.5 items-center justify-center rounded-full border-2 border-white bg-[#0073ea] text-white shadow-sm">
